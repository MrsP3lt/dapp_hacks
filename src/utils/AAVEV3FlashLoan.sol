...
library AAVEV3FlashLoan {
    ...
    function takeFlashLoan(address[] memory tokens, uint256[] memory amounts) internal {
        ...
        require(address(context1.AAVEPool) != address(0), "AAVEV3Flashloan: Query to AAVElendingPool failed.");
        uint256[] memory modes = new uint256[](tokens.length);

        context1.AAVEPool.flashLoan(address(this), tokens, amounts, modes, address(this), "", 0);
    }
    ...
    function payFlashLoan(bytes calldata data) internal {
        ...
        initiator;
        params;
        require(msg.sender == address(context1.AAVEPool), "AAVEV3FlashLoan: Callback msg.sender was not the pool");

        for (uint256 i = 0; i < assets.length; i++) {
            IERC20(assets[i]).approve(address(context1.AAVEPool), amounts[i] + premiums[i]);
        }
    }
