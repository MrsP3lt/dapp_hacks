...
library MakerDAOFlashLoan {
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        ...
        context1.dssFlash.flashLoan(address(this), token, amount, "");
    }
    ...
    function payFlashLoan(bytes calldata data) internal {
        (address initiator, address token, uint256 amount, uint256 fee, bytes memory userData) = unpackData(data);
        initiator;
        userData;
        Context memory context1 = context();

        require(msg.sender == address(context1.dssFlash), "MakerDAOFlashLoan: Callback msg.sender was not Maker DAO");
