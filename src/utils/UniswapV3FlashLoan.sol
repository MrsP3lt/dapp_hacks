...
library UniswapV3FlashLoan {
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        Context memory context1 = context(address(0), token, amount);

        require(address(context1.uniswapV3Pool) != address(0), "UniswapV3Flashloan: Pair contract not found");

        address token0 = IUniswapV3Pool(context1.uniswapV3Pool).token0();
        address token1 = IUniswapV3Pool(context1.uniswapV3Pool).token1();

        token1;
        uint256 amount0;
        uint256 amount1;

        (amount0, amount1) = token0 == context1.asset ? (amount, amount1) : (amount0, amount);

        IUniswapV3Pool(context1.uniswapV3Pool).flash(
    ...
    function takeFlashLoan(address pair, address token, uint256 amount) internal {
        Context memory context1 = context(pair, token, amount);
    ...
    function getFees() internal pure returns (FeesTickSpacing[] memory) {
        ...
        fees[3] = FeesTickSpacing(10_000, 200);
        return fees;
    }
