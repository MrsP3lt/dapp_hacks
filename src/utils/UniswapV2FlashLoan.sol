...
library UniswapV2FlashLoan {
    ...
    function takeFlashLoan(address[] memory tokens, uint256[] memory amounts) internal {
        ...
        Context memory context1 = context(tokens);

        require(address(context1.uniswapV2Pair) != address(0), "UniswapV2Flashloan: Pair contract not found");

        address token0 = IUniswapV2Pair(context1.uniswapV2Pair).token0();
        address token1 = IUniswapV2Pair(context1.uniswapV2Pair).token1();
        ...
        IUniswapV2Pair(context1.uniswapV2Pair).swap(
